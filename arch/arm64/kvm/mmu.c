			    ...
			    u64 size, bool may_block)
{
	...
	if (kvm_is_realm(kvm))
		kvm_realm_unmap_range(kvm, start, size, may_block);
	else
...
static int gmem_abort(const struct kvm_s2_fault_desc *s2fd)
{
	...
	bool is_private = kvm_is_realm(kvm) &&
	...
	if (kvm_vm_is_protected(vcpu->kvm)) {
		...
		if (kvm_slot_has_gmem(memslot) &&
		    (kvm_memslot_is_gmem_only(memslot) ||
		     !kvm_is_realm(vcpu->kvm) ||
		     private_ipa_fault(vcpu->kvm, fault_ipa)))
			ret = gmem_abort(&s2fd);
		else
...
{
	phys_addr_t start = range->start << PAGE_SHIFT;
	phys_addr_t size = (range->end - range->start) << PAGE_SHIFT;

	if (!kvm->arch.mmu.pgt || kvm_vm_is_protected(kvm))
		return false;

	if (kvm_is_realm(kvm)) {
		kvm_realm_unmap_range_filter(kvm, start, size,
					     range->may_block,
					     range->attr_filter);
	} else {
		__unmap_stage2_range(&kvm->arch.mmu, start, size,
				     range->may_block);
	}
