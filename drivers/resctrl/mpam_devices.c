...
static void mpam_init_reset_cfg(struct mpam_msc_ris *ris, struct mpam_config *reset_cfg)
{
	struct mpam_msc *msc = ris->vmsc->msc;
	...
	if (mpam_has_quirk(T241_FORCE_MBW_MIN_TO_ONE, msc)) {
		...
		reset_cfg->mbw_min = min_hw_granule + 1;
	}
}
...
static int mpam_reset_ris(void *arg)
{
	...
	mpam_init_reset_cfg(ris, &reset_cfg);
